// Backend API lives on a different domain (Render) than this frontend (Vercel),
...
async function loadYuvaks(){
  const res = await fetch(`${API_BASE}/api/yuvaks`);
...
async function loadRulebook(){
  const res = await fetch(`${API_BASE}/api/rulebook`);
