// Backend API lives on a different domain (Render) than this admin page (Vercel),
// so every request needs an absolute URL instead of a relative one.
...
$('#teamCsvInput').addEventListener('change', async e => {
  ...
  try{
    const res = await fetch(API_BASE + '/api/teams/import', { method:'POST', body: fd, credentials:'include' });
...
$('#yuvakCsvInput').addEventListener('change', async e => {
  ...
  try{
    const res = await fetch(API_BASE + '/api/yuvaks/import', { method:'POST', body: fd, credentials:'include' });
    const data = await res.json();
    if(!res.ok) throw new Error(data.error);
